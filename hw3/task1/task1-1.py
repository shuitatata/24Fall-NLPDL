...
def golden_greedy_decoding_wo_cache(batch):
    ...
    start_time = time.time()
    for timestep in range(MAX_NEW_LENGTH):
        tokenized_batch = original_model.prepare_inputs_for_generation(**tokenized_batch, use_cache=False)
...
@torch.no_grad()
def golden_greedy_decoding_with_cache(batch):
    tokenized_batch = tokenizer(batch, return_tensors="pt", padding=True, truncation=True, max_length=128).to('cuda')
    res = tokenized_batch['input_ids']
    ...
    start_time = time.time()
    for timestep in range(MAX_NEW_LENGTH):
        tokenized_batch = original_model_cached.prepare_inputs_for_generation(**tokenized_batch, use_cache=True, past_key_values=past_key_values)
        outputs = original_model_cached(**tokenized_batch)
        output_tokens = torch.argmax(outputs['logits'][:,-1], dim=-1, keepdim=True)
        tokenized_batch = {
            "input_ids": torch.cat([tokenized_batch['input_ids'], output_tokens], dim=-1),
            "attention_mask": torch.cat([tokenized_batch['attention_mask'], torch.ones_like(output_tokens)], dim=-1),
...
if __name__ == "__main__":
    ...
    original_model_cached = AutoModelForCausalLM.from_pretrained("openai-community/gpt2", attn_implementation="eager", device_map='cuda')
    ...
    for i in range(0, (len(prompt_dataset) + bsz - 1) // bsz):
        ...
        golden_with_cache_res, golden_with_cache_time = golden_greedy_decoding_with_cache(batch)
        # custom_res, custom_time = customized_greedy_decoding(batch)

        times[0] += golden_wo_cache_time
        # times[1] += custom_time
        times[2] += golden_with_cache_time

        # assert torch.equal(golden_wo_cache_res, custom_res), "Decoding results are not equal"
        assert torch.equal(golden_with_cache_res, golden_wo_cache_res), "Decoding results are not equal"
    ...
    print("Time taken for golden greedy decoding with KV cache: ", times[2])
